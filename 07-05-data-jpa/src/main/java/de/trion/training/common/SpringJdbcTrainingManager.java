...
public class SpringJdbcTrainingManager implements TrainingManager {
   ...
   public Training save(Training training) {
      ...
      var key = insert.executeAndReturnKey(
            Map.of("COL_TOPIC", training.getTopic(),
                  ...
                  "COL_DESCRIPTION", training.getDescription()));
   ...
   public List<Training> findAll() {
      var query = "SELECT COL_ID, COL_LOCATION, COL_TOPIC, COL_INSTRUCTOR_NAME, COL_DESCRIPTION FROM TBL_TRAININGS";
      return jdbcTemplate.query(query, new RowMapper<Training>() {
         @Override
         public Training mapRow(ResultSet rs, int rowNum) throws SQLException {
            var training = Training.withLocation(rs.getString(2)).withTopic(rs.getString(3)).withInstructor(rs.getString(4)).withDescription(rs.getString(5)).build();
   ...
   public Training getOne(Integer id) {
      var query = " SELECT COL_ID, COL_TOPIC, COL_LOCATION, COL_INSTRUCTOR_NAME, COL_DESCRIPTION FROM TBL_TRAININGS WHERE COL_ID=?";
